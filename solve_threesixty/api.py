...
class Solve360:
    ...
    def connect(self, verb, uri, body='', type='json'):
        ...
        headers = {
            'Authorization': 'Basic %s' % base64.encodestring(":".join([self.user, self.password]).encode('utf-8')).replace('\n', ''),
        }
...
#            self.connection.close()
    ...
    def connectJSON(self, verb, uri, body_dict):
        ...
        '''
        payload = self.connect(verb, uri, json.dumps(clearEmpties(body_dict), separators=(',',':')), type='json')
        self.close()
        return payload
